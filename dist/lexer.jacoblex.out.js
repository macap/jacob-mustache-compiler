...
function CDFA_mustag(){
	this.ss=1;
	this.as=[2,3,5,6];
	this.tt=[null,null,2,3,null,3,4];
this.stt={};
}
CDFA_mustag.prototype= new CDFA_base();
CDFA_mustag.prototype.nextState = function(state, c){
    var next = 0;
    switch(state){
case 1:
if(("#" === c ) || ("/" === c ) || ("^" === c )){
next = 2;
} else if(("." === c ) || ("0" <= c && c <= "9")  || ("a" <= c && c <= "z") ){
next = 3;
} else if(("}" === c )){
next = 4;
}
...
this.actions = [function anonymous(
 ...
 this.pushState('mustag'); return 'MUSTAG_START'; 
},function anonymous(
) {
 return this.jjtext; 
},function anonymous(
 ...
 this.popState(); return 'MUSTAG_END'; 
},function anonymous(
) {
 return 'TEXT';
},function anonymous(
...
this.states["mustag"] = {};
this.states["mustag"].dfa = new CDFA_mustag();
}
